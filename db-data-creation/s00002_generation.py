...
def generate_data(wikidata_info: pd.DataFrame):
    ...
    for row in wikidata_info.iterrows():
        ...
        if ADDRESS.format(row[1][1]) == row[1][0] or ADDRESS.format(row[1][5]) == row[1][4]:
            continue
        data.append({
            "text" : sentences[i // COUNT_PER_SENTENCE_TYPE].format(ct=row[1][1], co=row[1][5]),
            "logical_form" : "Court(\\a)^CourtName(\\a, {ct})^Region(\\a, {co})".format(ct=row[1][1], co=row[1][3])
        })
        i += 1
...
if __name__=="__main__":
    ...
    with open("s00002_sample.json", "w") as f:
        sampled_data = []
